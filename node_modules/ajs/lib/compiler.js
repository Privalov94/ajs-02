...
proto[Node.NEW] = function (ctor, args) {
  ...
  return this._format(["new", this._parenthesize(ctor, Node.SEQUENCE, Node.BINARY, Node.TERNARY, Node.ASSIGN, function (expr) {
    var w = this._astWalker(),
        has_call = {};
    try {
      w.withWalkers({
        "N_CALL": function N_CALL() {
          throw has_call;
        },
        "N_FUNCTION": function N_FUNCTION() {
          return this;
        }
      }, function () {
        w.walk(expr);
...
proto[Node.BREAK] = function (label) {
  var out = "break";
  if (label != null) out += " " + this[Node.NAME](label);
  return out + ";";
};
...
proto[Node.TERNARY] = function (co, th, el) {
  return this._format([this._parenthesize(co, Node.ASSIGN, Node.SEQUENCE, Node.TERNARY), "?", this._parenthesize(th, Node.SEQUENCE), ":", this._parenthesize(el, Node.SEQUENCE)]);
};
...
proto[Node.DOT] = function (expr) {
  var out = this._make(expr),
      i = 1;
  if (expr.type == Node.NUMBER) {
    if (!/\./.test(expr.children[0])) out += ".";
  } else if (this._needsParens(expr)) out = "(" + out + ")";
...
proto[Node.FOR_IN] = function (vvar, key, hash, block) {
  return this._format(["for", "(" + (vvar ? this._make(vvar).replace(/;+$/, "") : this._make(key)), "in", this._make(hash) + ")", this._make(block)]);
};
...
proto[Node.BINARY] = function (op, lvalue, rvalue) {
  var left = this._make(lvalue),
      right = this._make(rvalue);
  if (member(lvalue.type, [Node.ASSIGN, Node.TERNARY, Node.SEQUENCE]) || lvalue.type == Node.BINARY && g.precedence(op) > g.precedence(lvalue.children[0])) {
    left = "(" + left + ")";
  }
  if (member(rvalue.type, [Node.ASSIGN, Node.TERNARY, Node.SEQUENCE]) || rvalue.type == Node.BINARY && g.precedence(op) >= g.precedence(rvalue.children[0]) && !(rvalue.children[0] == op && member(op, ["&&", "||", "*"]))) {
    right = "(" + right + ")";
  }
...
proto[Node.UNARY_PREFIX] = function (op, expr) {
  ...
  if (!(expr.type == Node.NUMBER || expr.type == Node.UNARY_PREFIX && !g.is_operator(op + expr.children[0]) || !this._needsParens(expr))) val = "(" + val + ")";
  return op + (g.is_alphanumeric(op.charAt(0)) ? " " : "") + val;
};

proto[Node.UNARY_POSTFIX] = function (op, expr) {
  var val = this._make(expr);
  if (!(expr.type == Node.NUMBER || expr.type == Node.UNARY_POSTFIX && !g.is_operator(op + expr.children[0]) || !this._needsParens(expr))) val = "(" + val + ")";
  return val + op;
};

proto[Node.SUBSCRIPT] = function (expr, subscript) {
  var hash = this._make(expr);
  if (this._needsParens(expr)) hash = "(" + hash + ")";
  return hash + "[" + this._make(subscript) + "]";
};

proto[Node.OBJECT] = function (exprList) {
  if (exprList.children.length == 0) return "{}";
  var self = this;
  return "{" + MAP(exprList.children, function (p) {
    ...
    var key = p[0],
        val = self._make(p[1]);
    if ((typeof key == "number" || +key + "" == key) && parseFloat(key) >= 0) {
...
Compiler.prototype._blockStatements = function (statements) {
  for (var a = [], last = statements.length - 1, i = 0; i <= last; ++i) {
    ...
    if (code != ";") {
      if (i == last) {
        if (stat.type == Node.WHILE && emptyBlock(stat.children[1]) || member(stat.type, [Node.FOR, Node.FOR_IN]) && emptyBlock(stat.children[3]) || stat.type == Node.IF && emptyBlock(stat.children[1]) && !stat.children[2] || stat.type == Node.IF && stat.children[2] && emptyBlock(stat.children[2])) {
          code = code.replace(/;*\s*$/, ";");
        } else {
          code = code.replace(/;+\s*$/, "");
        }
...
Compiler.prototype._switchBlock = function (body) {
  ...
  return "{" + MAP(body, function (branch, i) {
    var has_body = branch[1].length > 0,
        code = (branch[0] ? this._format(["case", this._make(branch[0]) + ":"]) : "default:") + (has_body ? this._blockStatements(branch[1]).join('') : "");
  ...
  while (true) {
    ...
    if (type == Node.IF) {
      if (!b.children[2])
        ...
        return this._make(new Node(Node.BLOCK, th));
      b = b.children[2];
    } else if (type == Node.WHILE || type == Node.DO) b = b.children[1];else if (type == Node.FOR || type == Node.FOR_IN) b = b.children[3];else break;
  }
...
Compiler.prototype._parenthesize = function (expr) {
  ...
  for (var i = 1; i < arguments.length; ++i) {
    var el = arguments[i];
    if (el instanceof Function && el.apply(this, [expr]) || expr.type == el) return "(" + gen + ")";
  }
  return gen;
};
...
Compiler.prototype._needsParens = function (node) {
  if (node.type == Node.FUNCTION || node.type == Node.OBJECT) {
    var a = slice(this.stack),
        self = a.pop(),
        p = a.pop();
    while (p) {
      if (p.type == Node.STATEMENT) return true;
      if (member(p.type, [Node.SEQUENCE, Node.CALL, Node.DOT, Node.SUBSCRIPT, Node.TERNARY]) && p.children[0] === self || member(p.type, [Node.BINARY, Node.ASSIGN, Node.UNARY_POSTFIX]) && p.children[1] === self) {
        self = p;
        p = a.pop();
      } else {
        return false;
...
Compiler.prototype._optimizeOutputNodes = function (statements) {
  ...
  function addStat(stat) {
    if (binStat.children.length < 3) binStat.push(stat);else binStat = new Node(Node.BINARY, '+', binStat, stat);
  }
...
Compiler.prototype._astWalker = function (ast) {
  var user = {},
      ...
      self = this,
      walkers = {
    "N_NAME": function N_NAME(name) {
      return [this.type, name]; // maybe needs a new node?
    },
    "N_DOT": function N_DOT(expr) {
...
var DOT_CALL_NO_PARENS = g.array_to_hash([Node.NAME, Node.ARRAY, Node.OBJECT, Node.STRING, Node.DOT, Node.SUBSCRIPT, Node.CALL, Node.REGEXP]);

function needsCbWrap(func) {
  if (g.is_syncronous_call(func.children.slice(-1)[0])) return false;

  if (func.children[0].children && func.children[0].children[0] == '_') return false;
