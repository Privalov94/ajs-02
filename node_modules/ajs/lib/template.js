...
VM.prototype.render = function (locals, cb) {
  if (cb) {
    var buffer = [];
...
function resolveFilename(filename) {
  var cached;
  if (cached = filenameCache[filename]) return cached;else return filenameCache[filename] = path.resolve(process.cwd(), filename);
}

var dirnameCache = {};
function resolveDirname(filename) {
  var cached;
  if (cached = dirnameCache[filename]) return cached;else return dirnameCache[filename] = path.dirname(filename);
}
